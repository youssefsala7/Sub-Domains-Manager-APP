...
export class CoolifyAPI {
    ...
    constructor(apiKey, baseUrl) {
        if (!apiKey || !baseUrl) {
            throw new CoolifyAPIError('API key and base URL are required', 'INVALID_CREDENTIALS');
        }
    ...
    handleError(error, context) {
        if (axios.isAxiosError(error)) {
            ...
            throw new CoolifyAPIError(`${context}: ${data?.message ||
                error.message ||
                'An error occurred with the Coolify API'}`, 'API_ERROR', status, data);
        }
        console.error(`Coolify API Error (${context}):`, error);
        throw new CoolifyAPIError(`${context}: ${error.message || 'An unexpected error occurred'}`, 'UNKNOWN_ERROR', 500);
    }
    ...
    async createDeployment(params) {
        ...
        try {
            ...
            if (!process.env.TEMPLATE_REPO) {
                throw new CoolifyAPIError('GitHub repository URL is required', 'INVALID_PARAMETER');
            }
            if (!process.env.COOLIFY_PROJECT_ID ||
                !process.env.COOLIFY_ENVIRONMENT_ID ||
                !process.env.COOLIFY_SERVER_ID) {
                throw new CoolifyAPIError('Coolify project ID, environment ID, and server ID are required but not provided', 'INVALID_PARAMETER');
            }
            ...
            if (existingApp) {
                ...
                console.log('Creating template-based deployment with', clientData.deploymentType === 'custom-html'
                    ? 'custom HTML'
                    : 'link tree');
                const createPayload = {
                ...
                const createResponse = await axios.post(`${this.baseUrl}/api/v1/applications/public`, createPayload, {
                    headers: {
                        ...this.headers,
                ...
                if (!applicationId) {
                    throw new CoolifyAPIError('Application created but no ID returned', 'INVALID_RESPONSE');
                }
            }
            // Set environment variables
            console.log('Setting environment variables for application:', applicationId);
            ...
            if (clientData.htmlCode && clientData.htmlCode.trim()) {
                ...
                const encodedHTML = Buffer.from(clientData.htmlCode, 'utf8').toString('base64');
                envVariables.push({
            ...
            await axios.patch(`${this.baseUrl}/api/v1/applications/${applicationId}/envs/bulk`, {
                data: envVariables,
            }, {
                headers: this.headers,
            });
            ...
            await axios.get(`${this.baseUrl}/api/v1/deploy?uuid=${applicationId}`, {
                headers: this.headers,
            });
    ...
    async deleteDeployment(subdomain) {
        try {
            ...
            if (!app) {
                throw new CoolifyAPIError('Application not found', 'NOT_FOUND', 404);
            }
            console.log('Deleting application:', app.uuid);
            await axios.delete(`${this.baseUrl}/api/v1/applications/${app.uuid}`, {
                headers: this.headers,
            });
    ...
    async updateDeployment(params) {
        ...
        try {
            ...
            if (!app) {
                console.log('Application not found, creating new deployment...');
            ...
            await axios.patch(`${this.baseUrl}/api/v1/applications/${app.uuid}/envs/bulk`, {
                data: envVariables,
            }, {
                headers: this.headers,
            });
