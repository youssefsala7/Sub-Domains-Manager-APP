...
router.post('/:clientId/deploy', auth, async (req, res) => {
    try {
        ...
        const godaddyResponse = await axios.put(`${GODADDY_API_URL}/domains/${BASE_DOMAIN}/records/A/${client.subdomain}`, [record], {
            headers: {
        ...
        // Deploy to Coolify
        const coolifyResponse = await axios.post(`${COOLIFY_API_URL}/deployments`, {
            name: client.subdomain,
...
router.delete('/:clientId/deploy', auth, async (req, res) => {
    try {
        const client = await Client.findOne({
            _id: req.params.clientId,
            owner: req.user?.userId,
        ...
        const godaddyResponse = await axios.delete(`${GODADDY_API_URL}/domains/${BASE_DOMAIN}/records/A/${client.subdomain}`, {
            headers: {
                Authorization: `sso-key ${GODADDY_API_KEY}:${GODADDY_API_SECRET}`,
            },
        });
        ...
        const coolifyResponse = await axios.delete(`${COOLIFY_API_URL}/deployments/${client.subdomain}`, {
            headers: {
                Authorization: `Bearer ${COOLIFY_API_KEY}`,
            },
        });
...
router.get('/check/:subdomain', auth, async (req, res) => {
    try {
        ...
        try {
            ...
            await axios.get(`${GODADDY_API_URL}/domains/${BASE_DOMAIN}/records/A/${subdomain}`, {
                headers: {
                    Authorization: `sso-key ${GODADDY_API_KEY}:${GODADDY_API_SECRET}`,
                },
            });
