...
export let coolifyAPI = null;
export let cloudflareAPI = null;
// Initialize services at startup
function initializeServices() {
    // Check Coolify configuration
    if (!process.env.COOLIFY_API_KEY || !process.env.COOLIFY_API_URL) {
        console.error('Missing Coolify API configuration. Required environment variables:');
        console.error('- COOLIFY_API_KEY');
        console.error('- COOLIFY_API_URL');
    }
    else {
        try {
            coolifyAPI = new CoolifyAPI(process.env.COOLIFY_API_KEY, process.env.COOLIFY_API_URL);
    ...
    if (!process.env.CLOUDFLARE_API_TOKEN ||
        ...
        !process.env.DOMAIN) {
        console.error('Missing Cloudflare API configuration. Required environment variables:');
        ...
        try {
            cloudflareAPI = new CloudflareAPI(process.env.CLOUDFLARE_API_TOKEN, process.env.CLOUDFLARE_ZONE_ID, process.env.DOMAIN);
...
mongoose
    .connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/subdomain_manager')
    .then(() => {
