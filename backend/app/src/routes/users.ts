...
import { IUser, AuthRequest } from '../types.js';
...
router.get('/', auth, adminAuth, async (_req: AuthRequest, res) => {
	try {
...
router.patch('/:id/status', auth, adminAuth, async (req: AuthRequest, res) => {
	try {
		const { id } = req.params;
